...
class hyperestimator_fitness(base_ff):
    ...
    def evaluate(self, ind, **kwargs):
        with tf.device('/cpu:0'): # change this to GPU if you are running on a GPU
            ...
            train_generator = train_datagen.flow_from_directory('dataset/train',target_size=(224, 224),class_mode='categorical', batch_size=2)  # since we use binary_crossentropy loss, we need binary labels
            test_generator = train_datagen.flow_from_directory('dataset/test',target_size=(224, 224),class_mode='categorical')  # since we use binary_crossentropy loss, we need binary labels
            ...
            hist = model_final.fit_generator(generator= train_generator, 
                                 epochs= 5, validation_data= test_generator, steps_per_epoch = 1400 // int(a[2]),
                            validation_steps = 350 // int(a[2] )   
                                 )
